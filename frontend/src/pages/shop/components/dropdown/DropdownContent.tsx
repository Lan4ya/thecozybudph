...
>((props, forwardedRef) => {
  ...
  //       strategy: "fixed",
  //       middleware: [offset(sideOffset), flip(), shift({ crossAxis: true })],
  //     }).then(({ x, y }) => {
  //       setPos({
  //         position: "fixed",
  //         left: x,
  ...
  useLayoutEffect(() => {
    ...
    const updatePosition = () => {
      ...
      if (side === "bottom") {
        ...
        if (align === "start") next.left = Math.round(tr.left + alignOffset);
        else if (align === "center")
          next.left = Math.round(
            tr.left + tr.width / 2 - contentEl.offsetWidth / 2 + alignOffset,
          );
        else
          next.left = Math.round(
        ...
        next.top = Math.round(tr.top - contentEl.offsetHeight - sideOffset);
        if (align === "start") next.left = Math.round(tr.left + alignOffset);
        else if (align === "center")
          next.left = Math.round(
            tr.left + tr.width / 2 - contentEl.offsetWidth / 2 + alignOffset,
        ...
        next.left = Math.round(tr.left - contentEl.offsetWidth - sideOffset);
      ...
      {open && (
        ...
        >
          <div
            ...
            data-side={side}
            className={cn(
              "bg-popover text-popover-foreground min-w-32 overflow-x-hidden overflow-y-auto shadow-md rounded-md border p-1",
              className,
              getOrigin(),
            )}
            style={{ ...style }}
