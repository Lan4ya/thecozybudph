...
// INFO: Roll back implementation example in cases where payment status is 'successful' in paymongo's side but our DB didn't sync properly (e.g still pending). This can happen due to endpoint downtime, DNS issues, timeouts, connection errors, etc.
