  ...
  return (
    <div className="divide-y divide-border/30">
      {orderItems.map((item, idx) => (
        <OrderItemRow key={item.variantId} item={item} idx={idx} />
      ))}
    </div>
  );
};

const OrderItemRow = ({ item, idx }: { item: OrderItemUI; idx: number }) => {
  const [isExpanded, setIsExpanded] = useState(false);

  const cardMessages = item.cardMessages.filter((msg) => msg.trim() !== "");
