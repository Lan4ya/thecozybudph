...
import { OrderRepository } from "@shared/domain/orders/mod.ts";
import { PaymentRepository } from "@shared/domain/payments/mod.ts";
...
export const handlePayMongoWebhook = async (
  ...
  switch (eventType) {
    case "payment.paid": {
      // pending → paid
      const updatedPaidPayment = await PaymentRepository.updatePayment(
        supabase,
        paymentIntentId,
        {
          status: "paid",
      ...
      console.log("Payment update result:", updatedPaidPayment);

      const updatedOrder = OrderRepository.updateOrderStatus(
        supabase,
        updatedPaidPayment.order_id,
        "confirmed",
      );
